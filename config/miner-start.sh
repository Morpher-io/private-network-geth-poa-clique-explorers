#!/bin/sh
sleep 30 #cloudmap needs that to register the domain
...
geth --config /app/config.toml --verbosity 3 --light.serve 50 --txlookuplimit 0 --miner.etherbase=$SIGNERADD --mine --miner.gasprice 1 --unlock $SIGNERADD --password <(echo $SIGNERPW) -nodekey <(echo ${SIGNERKEY:?NOT SET})  --identity "Morpher Dev Demo Network"
